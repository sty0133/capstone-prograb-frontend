...
<template>
  <div class="chatbox">
    ...
    <p class="warning-message">PDF BOT은 실수를 할 수 있습니다. 중요한 정보는 재차 확인하세요.</p>
  </div>
</template>
...
const sendMessage = async () => {
  ...
  const userInput = newMessage.value // 사용자의 입력을 다른 변수에 저장
  newMessage.value = '' // 인풋 초기화
  isSending.value = true // 메시지 전송 시작
  emit('updateIsSending', true)

  messages.value.push({ text: userInput, type: 'user' }) // 저장된 입력값 사용
  scrollToBottom()

  try {
    const response = await apiClient.post('/pdf/rag', {
      question: userInput,
      chat_id: props.chatId, // chatId를 요청에 포함
    })

    if (response.data.status === 'success') {
      messages.value.push({ text: formatMessage(response.data.data.response), type: 'bot' })
    } else {
      console.error('에러 발생:', response.data.message)
    }
  } catch (error) {
    userStore.handleTokenError(error)
  }

  isSending.value = false // 메시지 전송 완료
  scrollToBottom()
  ...
  if (props.chatId) {
    try {
      const response = await apiClient.get('/user/history', {
        params: { chat_id: props.chatId }, // chatId를 요청에 포함
      })
      ...
      if (response.data.status === 'success') {
        messages.value = response.data.data.map((message) => ({
          ...
          timestamp: message.timestamp, // 필요하면 추가
        }))
      } else {
        console.error('대화 기록을 불러오는 중 에러 발생:', response.data.message)
      }
    } catch (error) {
      userStore.handleTokenError(error)
    }
